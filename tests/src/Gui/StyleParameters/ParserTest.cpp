...
TEST_F(ParserTest, InsetsNamedOverrideBeatsTupleArgument)
{
    ...
    Parser parser("padding((1px, 2px), top: 9px)");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
    const auto& tuple = result.get<Tuple>();
    EXPECT_EQ(tuple.kind, TupleKind::Padding);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("top").value, 9.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("right").value, 2.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("bottom").value, 1.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("left").value, 2.0);
}

TEST_F(ParserTest, InsetsGroupOverrideBeatsTupleArgument)
{
    Parser parser("padding(margins(1px, 2px, 3px, 4px), vertical: 9px)");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
    const auto& tuple = result.get<Tuple>();
    ...
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("top").value, 9.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("right").value, 2.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("bottom").value, 9.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("left").value, 4.0);
}
...
TEST_F(ParserTest, ExtraNamedElementDropsKind)
{
    ...
    Parser parser("padding(10px) + (foo: 5px)");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
    const auto& tuple = result.get<Tuple>();
...
TEST_F(ParserTest, UnmatchedUnnamedElementsDropKind)
{
    ...
    Parser parser("padding(10px) + (1px, 2px, 3px, 4px)");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
...
TEST_F(ParserTest, BorderRadiusNamedOverrideBeatsTupleArgument)
{
    ...
    Parser parser("border_radius((1px, 2px), top_left: 9px)");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
    const auto& tuple = result.get<Tuple>();
    EXPECT_EQ(tuple.kind, TupleKind::Corners);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("top_left").value, 9.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("top_right").value, 2.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("bottom_right").value, 1.0);
    EXPECT_DOUBLE_EQ(tuple.get<Numeric>("bottom_left").value, 2.0);
}
...
{
    {
        ...
        Value result;
        EXPECT_NO_THROW({ result = expr->evaluate({.manager = &manager, .context = {}}); });
        ASSERT_TRUE(result.holds<Tuple>());
...
TEST_F(ParserTest, GradientAcceptsPercentagesAsNormalizedCoordinates)
{
    ...
    Parser parser("linear_gradient(x2: 100%, (0%, #ff0000), (50%, #0000ff))");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
...
TEST_F(ParserTest, GradientGeometryWithUnitReportsAndDefaults)
{
    ...
    Parser parser("linear_gradient(x2: 10px, #ff0000, #0000ff)");
    auto expr = parser.parse();
    auto result = expr->evaluate({.manager = &manager, .context = {}});
    ASSERT_TRUE(result.holds<Tuple>());
...
TEST_F(ParserTest, GradientStopPositionWithUnitReportsAndDegrades)
{
    ...
    Parser parser("linear_gradient((10px, #ff0000), (1, #0000ff))");
    auto expr = parser.parse();
...
{
    Parser percentageParser("shade(#ff0000, 80%)");
    const Value percentage = percentageParser.parse()->evaluate({.manager = &manager, .context = {}});

    Parser fractionParser("shade(#ff0000, 0.8)");
    const Value fraction = fractionParser.parse()->evaluate({.manager = &manager, .context = {}});
