...
def compute_mergeability_score(
    ...
    *** THIS FUNCTION IS INTENTIONALLY ISOLATED FOR EASY ADJUSTMENT ***
    ...
    factors["size_penalty"] = -(math.log(max(additions + deletions + 1, 1)) * 3)
...
def _filter_pr_item(item: dict, skip_labels: set[str], open_only: bool) -> tuple[str, dict] | None:
    """Return (item_id, pr_data) if the item passes all filters, else None.
    ...
    can be zeroed out, clearing stale values from the project board.
    """
...
def fetch_project_data(
...
) -> tuple[str, dict[str, str | None], list[tuple[str, dict]]]:
    ...
    When open_only is False, closed/merged PRs are included so stale scores can be zeroed.
    """
...
def main() -> int:
    ...
    for item_id, pr_data in pr_items:
        ...
        is_blocked = has_change_requested or pr_data.get("mergeable") == "CONFLICTING"
    ...
    if args.dry_run or args.show_all:
        ...
        log.info("Done. Updated %d PR(s).", len(results))
