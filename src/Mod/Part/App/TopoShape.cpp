...
TopoDS_Shape TopoShape::makeTube(double radius, double tol, int cont, int maxdegree, int maxsegm) const
{
    ...
    aCirc->Rotate (gp::OZ(), Base::numbers::pi/2.);
                                      ...
                                      Standard_Boolean leftHanded) const
{
    ...
    gp_Pnt2d aPnt(0, 0);
    gp_Dir2d aDir(2. * Base::numbers::pi, pitch);
    Standard_Real coneDir = 1.0;
    if (leftHanded) {
        aDir.SetCoord(-2. * Base::numbers::pi, pitch);
        coneDir = -1.0;
    }
    gp_Ax2d aAx2d(aPnt, aDir);
    Handle(Geom2d_Line) line = new Geom2d_Line(aAx2d);
    ...
    if (partTurn > Precision::Confusion()) {
        if (isCylinder) {
            end = line->Value(sqrt(4.0*Base::numbers::pi*Base::numbers::pi+pitch*pitch)*turns);
        }
        else {
            u = coneDir * turns * 2.0 * Base::numbers::pi;
            v = height / cos(angle);
            end = gp_Pnt2d(u, v);
        }
        segm = GCE2d_MakeSegment(beg , end);
                                   ...
                                   Standard_Real radius) const
{
    ...
    gp_Pnt2d aPnt(2. * Base::numbers::pi , height / 2.);
    gp_Dir2d aDir(2. * Base::numbers::pi , height / 4.);
    gp_Ax2d aAx2d(aPnt , aDir);

    Standard_Real aMajor = 2. * Base::numbers::pi;
    ...
    Handle(Geom2d_TrimmedCurve) aArc1 = new Geom2d_TrimmedCurve(anEllipse1 , 0 , Base::numbers::pi);
    Handle(Geom2d_TrimmedCurve) aArc2 = new Geom2d_TrimmedCurve(anEllipse2 , 0 , Base::numbers::pi);
