...
struct ShapeHasher
{
    ...
    inline size_t operator()(const std::pair<TopoShape, TopoShape>& s) const
    {
        ...
        size_t res = s.first.getShape().HashCode(max);
        hash_combine(res, s.second.getShape().HashCode(max));
#endif
        return res;
    }
...
#else
        const int max = std::numeric_limits<int>::max();
        size_t res = s.first.HashCode(max);
        hash_combine(res, s.second.HashCode(max));
#endif
        return res;
    }
