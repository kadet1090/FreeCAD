    ...
    , nodeset(new PartGui::SoBrepPointSet)
{
    ...
    SO_NODE_ADD_FIELD(transparency, (0.8f));
                                                      ...
                                                      SoPreviewShape* preview)
{
    ...
    try {
        ...
        Base::Console().userTranslatedNotification(
            tr("Failure while computing preview: %1. That usually indicates an error with model.").arg(message)
        );
        Base::Console().userWarning(
            QT_TRANSLATE_NOOP("Notifications", "Failure while rendering preview: %s. That usually indicates an error with model."),
            e.GetMessageString()
        );
