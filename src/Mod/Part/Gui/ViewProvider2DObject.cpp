...
App::PropertyQuantityConstraint::Constraints ViewProvider2DObjectGrid::GridSizeRange = {0.001, num_double::max(), 1.0};
...
SoSeparator* ViewProvider2DObjectGrid::createGrid()
{
    ...
    if (TightGrid.getValue()) {
        ...
        float xMin = std::abs(MinX) < eps ? 0.01f : MinX;
        float xMax = std::abs(MaxX) < eps ? 0.01f : MaxX;
        float yMin = std::abs(MinY) < eps ? 0.01f : MinY;
        float yMax = std::abs(MaxY) < eps ? 0.01f : MaxY;
