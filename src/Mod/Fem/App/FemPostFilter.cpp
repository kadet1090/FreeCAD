...
void FemPostContoursFilter::refreshFields()
{
    ...
    if (Field.isValid()) {
        const auto it = std::find(FieldsArray.begin(), FieldsArray.end(), fieldName);
...
DocumentObjectExecReturn* FemPostScalarClipFilter::execute()
{
    ...
    // search if the current field is in the available ones and set it
    const auto it = std::find(ScalarsArray.begin(), ScalarsArray.end(), val);
...
DocumentObjectExecReturn* FemPostWarpVectorFilter::execute()
{
    ...
    // search if the current field is in the available ones and set it
    const auto it = std::find(VectorArray.begin(), VectorArray.end(), val);
...
std::vector<std::string> FemPostCalculatorFilter::getScalarVariables() const
{
#if VTK_MAJOR_VERSION == 7
    std::vector<std::string> scalars;
    if (auto names = m_calculator->GetScalarVariableNames()) {
        scalars.reserve(m_calculator->GetNumberOfScalarArrays() + 3);
        for (int i = 0; i < m_calculator->GetNumberOfScalarArrays(); i++) {
            scalars.emplace_back(names[i]);
        }
    }
#else
...
std::vector<std::string> FemPostCalculatorFilter::getVectorVariables() const
{
#if VTK_MAJOR_VERSION == 7
    std::vector<std::string> vectors;
    if (auto names = m_calculator->GetVectorVariableNames()) {
        vectors.reserve(m_calculator->GetNumberOfVectorArrays() + 1);
        for (int i = 0; i < m_calculator->GetNumberOfVectorArrays(); i++) {
            vectors.emplace_back(names[i]);
        }
    }
#else
