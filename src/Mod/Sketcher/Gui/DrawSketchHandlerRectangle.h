        ...
        switch (state()) {
            ...
            case SelectMode::SeekSecond: {
                if (constructionMethod() == ConstructionMethod::Diagonal) {
                    ...
                    angle123 = Base::numbers::pi / 2;
                    angle412 = Base::numbers::pi / 2;
                }
            ...
            case SelectMode::SeekThird: {
                if (constructionMethod() == ConstructionMethod::Diagonal
                    ...
                    angle412 = Base::numbers::pi - angle123;
                    if (roundCorners) {
                        radius = std::min(length, width) / 6  // NOLINT
                    ...
                    angle123 = Base::numbers::pi - angle412;
                    if (roundCorners) {
                        radius = std::min(length, width) / 6  // NOLINT
                            * std::min(sqrt(1 - cos(angle412) * cos(angle412)),
                                       sqrt(1 - cos(angle123) * cos(angle123)));
...
private:
    ...
    void createShape(bool onlyeditoutline) override
    {
        ...
        if (length < Precision::Confusion() || width < Precision::Confusion()
            || fmod(fabs(angle123), Base::numbers::pi) < Precision::Confusion()) {
            return;
        }
    ...
    {
        double end = angle - Base::numbers::pi / 2;
        ...
        if (roundCorners && radiusFrame > Precision::Confusion()) {
            ...
            addArcToShapeGeometry(toVector3d(frameCorner1 + b1 * L2F),
                                  end - Base::numbers::pi + angle412,
                                  end,
                                  radiusFrame,
                                  isConstructionMode());
            addArcToShapeGeometry(toVector3d(frameCorner2 - b2 * L1F),
                                  end,
                                  end - Base::numbers::pi - angle123,
                                  radiusFrame,
                                  isConstructionMode());
    ...
    void addAlignmentConstraints()
    {
        ...
        if (fabs(angle) < Precision::Confusion()
            ...
            addToShapeConstraints(Sketcher::Parallel,
                                  ...
                                  Sketcher::PointPos::none,
                                  firstCurve + 3);
            if (fabs(angle123 - Base::numbers::pi / 2) < Precision::Confusion()) {
                addToShapeConstraints(Sketcher::Perpendicular,
                                      firstCurve,
                                      Sketcher::PointPos::none,
                                      firstCurve + 1);
        ...
        case SelectMode::SeekThird: {
            if (handler->constructionMethod() == ConstructionMethod::Diagonal
                ...
                if (onViewParameters[OnViewParameter::Sixth]->isSet) {
                    ...
                    if (fmod(angle, Base::numbers::pi) < Precision::Confusion()) {
                        unsetOnViewParameter(onViewParameters[OnViewParameter::Sixth].get());
                        return;
                    }
                ...
                if (onViewParameters[OnViewParameter::Sixth]->isSet) {
                    double c =
                        Base::toRadians(onViewParameters[OnViewParameter::Sixth]->getValue());
                    if (fmod(c, Base::numbers::pi) < Precision::Confusion()) {
                        unsetOnViewParameter(onViewParameters[OnViewParameter::Sixth].get());
                        return;
                    }

                    double a = asin(width * sin(Base::numbers::pi - c)
...
void DSHRectangleController::addConstraints()
{
    ...
    if (handler->constructionMethod() == ConstructionMethod::ThreePoints) {
        if (angleSet) {
            if (fabs(angle - Base::numbers::pi) < Precision::Confusion()
                || fabs(angle + Base::numbers::pi) < Precision::Confusion()
                || fabs(angle) < Precision::Confusion()) {
                Gui::cmdAppObjectArgs(obj,
                                      "addConstraint(Sketcher.Constraint('Horizontal',%d)) ",
                                      firstCurve);
            }
            else if (fabs(angle - Base::numbers::pi / 2) < Precision::Confusion()
                     || fabs(angle + Base::numbers::pi / 2) < Precision::Confusion()) {
                Gui::cmdAppObjectArgs(obj,
        ...
        if (innerAngleSet) {
            if (fabs(innerAngle - Base::numbers::pi / 2) > Precision::Confusion()) {
