...
void EditModeConstraintCoinManager::updateVirtualSpace()
{
    ...
    if (constrlist.size() == vConstrType.size()) {
        ...
        for (size_t i = 0; i < constrlist.size(); i++) {
            sws[i] = !(constrlist[i]->isInVirtualSpace != isshownvirtualspace);  // XOR of constraint
                                                                                 // mode and VP mode
        }
            ...
            switch (Constr->Type) {
                ...
                case Perpendicular: {
                    ...
                    if (Constr->Third != GeoEnum::GeoUndef ||            // perpty via point
                        ...
                        norm1.Normalize();
                        dir1 = norm1;
                        dir1.RotateZ(-Base::numbers::pi / 2.0);
                    }
                        ...
                        if (geo1->is<Part::GeomLineSegment>()) {
                            ...
                            norm1 = Base::Vector3d(
                                cos(Base::numbers::pi / 4),
                                sin(Base::numbers::pi / 4),
                                0
                            );
                            dir1 = Base::Vector3d(-norm1.y, norm1.x, 0);
                    ...
                    if (Constr->SecondPos != Sketcher::PointPos::none) {
                        ...
                        if (isLineSegment(*geo)) {
                            ...
                            asciiText->string = SbString(
                                std::string("◠ ").append(getPresentationString(Constr).toUtf8()).c_str()
                            );
                    ...
                    if (Constr->Second != GeoEnum::GeoUndef) {
                        ...
                        if (isArcOfCircle(*geo1) && Constr->FirstPos == Sketcher::PointPos::none) {
                            ...
                            if (helperRange1 != 0.) {
                                ...
                                helperRange1 = 2 * Base::numbers::pi - (endAngle - startAngle);
                        ...
                        if (isArcOfCircle(*geo2) && Constr->SecondPos == Sketcher::PointPos::none) {
                            ...
                            if (helperRange2 != 0.) {
                                ...
                                helperRange2 = 2 * Base::numbers::pi - (endAngle - startAngle);
                ...
                case SnellsLaw: {
                    ...
                    if (
                        ...
                        dir.RotateZ(-Base::numbers::pi / 2.0);
                ...
                case Angle: {
                    ...
                    if (Constr->Second != GeoEnum::GeoUndef) {
                        ...
                        if (Constr->Third == GeoEnum::GeoUndef) {  // angle between two lines
                            ...
                            // p.x, p.y);
                            dir1.RotateZ(-Base::numbers::pi / 2);  // convert to vector of tangency
                                                                   // by rotating
...
QString EditModeConstraintCoinManager::getPresentationString(const Constraint* constraint)
{
    ...
    valueStr = QString::fromStdString(
        constraint->getPresentationValue().getUserString(factor, unitStr)
    );
    ...
    if (constraintParameters.bHideUnits && constraint->Type != Sketcher::Angle) {
        ...
        if (!baseUnitStr.empty()) {
            ...
            if (baseUnitStr.compare(unitStr) == 0) {
                ...
                QRegularExpression rxUnits(QStringLiteral(" \\D*$"));  // space + any non digits at
                                                                       // end of string
