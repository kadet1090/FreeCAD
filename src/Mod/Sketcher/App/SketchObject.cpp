...
void SketchObject::reverseAngleConstraintToSupplementary(Constraint* constr, int constNum)
{
    ...
    if (constraintHasExpression(constNum)) {
        ...
        constr->setValue(GCS::pi - actAngle);
    }
}
...
int SketchObject::trim(int GeoId, const Base::Vector3d& point)
{
    ...
    if (!isClosedCurve(geoAsCurve)) {
        idsOfOldConstraints.erase(std::remove_if(idsOfOldConstraints.begin(), idsOfOldConstraints.end(),
            [&GeoId, &allConstraints, &cuttingGeoIds](const auto& i) {
                ...
                return !involvesBothButNotBothKept && ((involvesStart && keepStart) || (involvesEnd && keepEnd));
    ...
    newConstraints.erase(
        std::remove_if(
            newConstraints.begin(),
            newConstraints.end(),
            [](const auto& constr) { return constr->Type == ConstraintType::None; }
        ),
        newConstraints.end()
    );
    ...
    if (refIsLine) {
        ...
        for (auto geoId : geoIdList) {
            ...
            if (geosym->is<Part::GeomLineSegment>()) {
                ...
                double theta1 = Base::fmod(atan2(sep.y - scp.y, sep.x - scp.x), 2.f * GCS::pi);
                double theta2 = Base::fmod(atan2(ssp.y - scp.y, ssp.x - scp.x), 2.f * GCS::pi);
                ...
                theta1 = 2.0 * GCS::pi - theta1;
                theta2 = 2.0 * GCS::pi - theta2;
        ...
        for (auto geoId : geoIdList) {
            ...
            if (geosym->is<Part::GeomLineSegment>()) {
                ...
                double theta1 = Base::fmod(atan2(ssp.y - scp.y, ssp.x - scp.x), 2.f * GCS::pi);
                double theta2 = Base::fmod(atan2(sep.y - scp.y, sep.x - scp.x), 2.f * GCS::pi);
...
int SketchObject::exposeInternalGeometry(int GeoId)
{
    ...
    SketchGeometryType geomType{this};
    return geomType.exposeInternalGeometry(GeoId);
}
        ...
        if (vec1.IsParallel(vec2, Precision::Confusion())) {
            ...
            if (vec1.IsNormal(vec2, Precision::Angular())) {
                ...
                if (!curve.IsClosed()) {// arc of circle
                    ...
                    while (startAngle < -GCS::pi / 2.0 && tours < 10) {
                        startAngle = baseAngle + ++tours * 2.0 * GCS::pi + alpha;
                    }
                    while (startAngle >= 3.0 * GCS::pi / 2.0 && tours > -10) {
                        startAngle = baseAngle + --tours * 2.0 * GCS::pi + alpha;
                    }
                    ...
                    if (startAngle <= 0.0) {
                    ...
                    else if (startAngle < GCS::pi) {
                        if (endAngle < GCS::pi) {
                            P1 = ProjPointOnPlane_XYZ(beg, sketchPlane);
                            P2 = ProjPointOnPlane_XYZ(end, sketchPlane);
                        }
                        else if (endAngle < 2.0 * GCS::pi - startAngle) {
                            P2 = ProjPointOnPlane_XYZ(beg, sketchPlane);
                            // P1 = P1, already defined
                        }
                        else if (endAngle < 2.0 * GCS::pi) {
                            P2 = ProjPointOnPlane_XYZ(end, sketchPlane);
                            // P1 = P1, already defined
                        }
                        else {
                        ...
                        else if (endAngle < 4 * GCS::pi - startAngle) {
                            P1 = ProjPointOnPlane_XYZ(beg, sketchPlane);
                            // P2 = P2, already defined
                        }
                        else if (endAngle < 3 * GCS::pi) {
                            // P1 = P1, already defined
                            P2 = ProjPointOnPlane_XYZ(end, sketchPlane);
                        }
                        else {
            ...
            if (projection && !refSubShape.IsNull()) {
                ...
                case TopAbs_FACE: {
                    ...
                    if (surface.GetType() == GeomAbs_Plane) {
                        ...
                        if (fabs(dnormal.Angle(snormal) - GCS::pi/2) < Precision::Confusion()) {
    ...
    try {
        ...
        if (!bLock) {
            ...
            if (cstr->Type == Tangent) {
                angleOffset = -GCS::pi / 2;
                angleDesire = 0.0;
            }
            if (cstr->Type == Perpendicular) {
                angleOffset = 0;
                angleDesire = GCS::pi / 2;
            }
