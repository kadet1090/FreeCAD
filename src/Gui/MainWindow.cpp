...
void MainWindow::initDockWindows(bool show)
{
    ...
    // Task vew (task watcher).
    if (d->hiddenDockWindows.find("Std_TaskView") == std::string::npos) {
        //work through parameter.
        ParameterGrp::handle group = App::GetApplication().GetUserParameter().
              GetGroup("BaseApp/Preferences/DockWindows/TaskView");
        bool enabled = group->GetBool("Enabled", false);
        group->SetBool("Enabled", enabled); //ensure entry exists.
        _updateDockWidget("Std_TaskView", enabled, show, Qt::RightDockWidgetArea,
            [](QWidget *widget) {
                if(widget)
                    return widget;
                widget = new TaskView::TaskView(getMainWindow());
                widget->setObjectName(QStringLiteral(QT_TRANSLATE_NOOP("QDockWidget","Tasks")));
                return widget;
            });
    }
