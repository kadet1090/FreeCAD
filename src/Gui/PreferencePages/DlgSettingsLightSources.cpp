...
void DlgSettingsLightSources::loadSettings()
{
    ...
    const auto loadAngles = [&](QuantitySpinBox* horizontalAngleSpinBox,
                                QuantitySpinBox* verticalAngleSpinBox,
                                const char* parameter, const char* preset) {
        try {
            const auto direction = Base::stringToVector(hGrp->GetASCII(parameter, preset));
    ...
    // clang-format off
    loadAngles(ui->mainLightHorizontalAngle, ui->mainLightVerticalAngle, "HeadlightDirection", View3DSettings::defaultHeadLightDirection);
    loadAngles(ui->backLightHorizontalAngle, ui->backLightVerticalAngle, "BacklightDirection", View3DSettings::defaultBackLightDirection);
    loadAngles(ui->fillLightHorizontalAngle, ui->fillLightVerticalAngle, "FillLightDirection", View3DSettings::defaultFillLightDirection);
    // clang-format on
}
