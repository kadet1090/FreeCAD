...
static QImage createLinearGradient(const QSize&& size, const QList<QColor>& colors)
{
    const int w = size.width(), h = size.height();
    const QColor &fromColor = colors.first(), &toColor = colors.at(1), &midColor = colors.last();
...
static QImage createRadialGradient(const QSize&& size, const QList<QColor>& colors)
{
    const int w = size.width(), h = size.height();
    const QColor &fromColor = colors.first(), &toColor = colors.at(1), &midColor = colors.last();
    const qreal cx = static_cast<qreal>(w) / 2.0, cy = static_cast<qreal>(h) / 2.0,
...
static void horizontalShuffle(QImage& img, const qreal percent)
{
...
)
{
    Q_ASSERT_X(
        colors.count() == 3,
        "bgImage generation",
        "Three colors must be provided to create an image."
...
void SoFCBackgroundGradient::setupUpdater(void)
{
    ...
    auto createFun = [this, watcher]() {
        if (old_fCol == fCol && old_tCol == tCol && old_mCol == mCol && old_gradient == gradient) {
            return;
        }
