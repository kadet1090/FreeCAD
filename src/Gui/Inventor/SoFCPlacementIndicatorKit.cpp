...
SoSeparator* SoFCPlacementIndicatorKit::createAxes()
{
    ...
    const auto createAxis = [&](const char* label,
        ...
        auto rotation = SbRotation(SbVec3f(0.0, 1.0, 0.0), axis);
        auto pcTranslate = new SoTransform();
        pcTranslate->translation.setValue((cylinderOffset + offset) * axis);
        pcTranslate->rotation.setValue(rotation);
        ...
        if (coloredAxis.getValue()) {
            auto pcBaseColor = new SoBaseColor();
            pcBaseColor->rgb.setValue(axisColor);
...
SoSeparator* SoFCPlacementIndicatorKit::createPlaneIndicator()
{
    ...
    auto rotation = SbRotation(SbVec3f(0.0, 1.0, 0.0), SbVec3f(0.0, 0.0, 1.0));

    auto pcRotation = new SoRotation();
    pcRotation->rotation = rotation;
    ...
    const auto gray = SbColor(0.75f, 0.75f, 0.75f);

    auto pcMaterial = new SoMaterial();
    pcMaterial->diffuseColor.setValue(gray);
